   ...
   def _load_tinyllama_model_windows(self):
       ...
       try:
           ...
           for path in possible_paths:
               ...
               if path.exists():
                   model_path = path
                   logger.info(f"✅ Found model at: {path.absolute()}")
                   break
           ...
           # Validate model configuration files before loading
           ...
           if config_path.exists():
               try:
                   ...
                   with open(config_path, 'r') as f:
                       config = json.load(f)
                   logger.info("✅ Model config.json is valid")
               except json.JSONDecodeError as e:
                   logger.error(f"❌ Model config.json is corrupted: {e}")
                   return
               except Exception as e:
                   logger.error(f"❌ Error reading model config.json: {e}")
                   return
           else:
           ...
           # Cross-platform model loading with fallback methods
           if is_qwen:
               ...
               try:
                   ...
                   logger.warning(f"⚠️ Primary Qwen loading failed: {e}")
               ...
               try:
                   ...
                   logger.warning(f"⚠️ Primary Phi-3 loading failed: {e}")
                   # Fallback method for Linux
                   self.llm_model = AutoModelForCausalLM.from_pretrained(
                       str(model_path),
                       trust_remote_code=True,
               ...
               try:
                   ...
                   logger.warning(f"⚠️ Primary TinyLlama loading failed: {e}")
                   # Fallback method for Linux compatibility
                   try:
                       self.llm_model = AutoModelForCausalLM.from_pretrained(
                           str(model_path),
                           trust_remote_code=True,
                       ...
                       logger.error(f"❌ Fallback TinyLlama loading also failed: {e2}")
                       raise e2
